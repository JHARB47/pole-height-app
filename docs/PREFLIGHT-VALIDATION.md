...
```javascript
test("user fixes validation errors and successfully exports", async ({
  page,
}) => {
  // Navigate to outputs
  ...
  await expect(page.locator(".preflight-error")).toContainText(
    "Missing: Job Number",
  );
  ...
  await expect(page.locator(".preflight-status")).toContainText(
    "Ready to Export",
  );
...
### 10.3 Calculation Errors (ERR*CALC*\*)
