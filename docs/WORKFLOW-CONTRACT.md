...
function validateDataIntake(state, deliverables) {
  const needsPoles = deliverables.some(d => 
    ['gis_export', 'permit_report', 'firstenergy_export', 'field_collection'].includes(d)
  );
  ...
  if (deliverables.includes('gis_export')) {
    const polesWithCoords = state.collectedPoles?.filter(p => 
...
function validateExistingPlant(state, deliverables) {
  ...
  if (deliverables.some(d => ['permit_report', 'clearance_analysis'].includes(d))) {
...
function validateSpanModeling(state, deliverables) {
  ...
  const hasManualSpan = state.spanDistance && parseFloat(state.spanDistance) > 0;
...
function validateFieldCollection(state, deliverables) {
  ...
  const polesWithGPS = state.collectedPoles?.filter(p => 
    p.latitude && p.longitude
  ) || [];
