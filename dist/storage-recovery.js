...
(function() {
    ...
    try {
      ...
      relatedKeys.forEach(key => {
        try {
          ...
          error(`Failed to remove key ${key}:`, e instanceof Error ? e.message : String(e));
        }
      });
      
      return true;
    } catch (e) {
      error('Failed to clear storage:', e instanceof Error ? e.message : String(e));
      return false;
    }
  }
  ...
  function validateStorage() {
    ...
    try {
      ...
      error('localStorage is not available:', e instanceof Error ? e.message : String(e));
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        return clearStorage(`Invalid JSON: ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      try {
        ...
        return clearStorage(`State cannot be serialized: ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      error('Unexpected validation error:', e instanceof Error ? e.message : String(e));
      return clearStorage(`Validation error: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  ...
  window.__clearAppStorage = function() {
    ...
    if (cleared) {
      ...
      alert('Failed to clear storage. Please try clearing browser data manually.');
    }
  };
