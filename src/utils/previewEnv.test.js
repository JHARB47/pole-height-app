...
describe('previewEnv helpers', () => {
  it('detects Netlify branch deploy host', () => {
    ...
    expect(computeIsPreviewHost(host, 'poleplanpro.com', 'poleplanpro.netlify.app')).toBe(true);
  });
  ...
  it('does not treat apex as preview host', () => {
    ...
    expect(computeIsPreviewHost(host, 'poleplanpro.com', 'poleplanpro.netlify.app')).toBe(false);
  });

  it('hides auth buttons only when preview auth is disabled and host is preview', () => {
    const host = 'branch--poleplanpro.netlify.app';
    expect(computeHideAuthButtons(host, true, 'poleplanpro.com', 'poleplanpro.netlify.app')).toBe(true);
    expect(computeHideAuthButtons(host, false, 'poleplanpro.com', 'poleplanpro.netlify.app')).toBe(false);
  });
});
