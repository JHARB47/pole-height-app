...
const envs = [
  ...
  "residentialYard",
  "residentialDriveway",
  "nonResidentialDriveway",
  "waterway",
  "wvHighway",
  "interstate",
  "interstateNewCrossing",
  "railroad",
];
...
describe("randomized smoke: computeAnalysis resilience", () => {
  it("runs multiple times with realistic random inputs (<=60ft) without throwing", () => {
    for (let i = 0; i < 25; i++) {
      ...
      const existingPowerHeight = isNew
        ? ""
        : `${randInt(20, 35)}' ${randInt(0, 11)}"`;
      const attachmentType = pick(DEFAULTS.cableTypes).value;
      const windSpeed = String(randInt(60, 100));
      const env = pick(envs);
      const proposedLineHeight =
        Math.random() < 0.3 ? "" : `${randInt(12, 35)}' ${randInt(0, 11)}"`;
      ...
      if (out.errors) {
        ...
        expect(Number.isFinite(out.results.attach?.proposedAttachFt ?? 0)).toBe(
          true,
        );
      }
    }
