...
function polesToCSV(poles = []) {
  const headers = [
    'id', 'label', 'latitude', 'longitude', 'height_ft', 'class', 'attach_height_ft',
    'bearing_deg', 'pull_ft', 'notes'
  ];
  ...
  for (const s of spans) {
    if (s.coordinates && s.coordinates.length === 2) {
      features.push({
        ...
        properties: { id: s.id, from: s.fromPoleId, to: s.toPoleId, length_ft: s.lengthFt }
      });
    }
  }
...
export function buildExportBundle({ poles = [], spans = [], existingLines = [], preset = 'generic', job = { name: 'Export', jobNumber: '' }, includeBOM = false } = {}) {
...
export async function buildExportZip({
  ...
  for (const [filePath, fileData] of Object.entries(files)) zip.file(filePath, fileData);
