...
export function validatePoles(poles) {
  if (!Array.isArray(poles)) return { errors: ['Poles must be an array'], data: [] };
  const errors = [];
  const data = [];
  poles.forEach((p, idx) => {
    const r = PoleSchema.safeParse(p);
    if (r.success) data.push(r.data); else errors.push(`Pole[${idx}]: ${r.error.issues.map(i => i.message).join('; ')}`);
  });
  return { errors, data };
}

export function validateSpans(spans) {
  if (!Array.isArray(spans)) return { errors: ['Spans must be an array'], data: [] };
  const errors = [];
  const data = [];
  spans.forEach((s, idx) => {
    const r = SpanSchema.safeParse(s);
    if (r.success) data.push(r.data); else errors.push(`Span[${idx}]: ${r.error.issues.map(i => i.message).join('; ')}`);
  });
  return { errors, data };
}
