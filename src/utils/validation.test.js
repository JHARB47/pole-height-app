...
describe('validation helpers', () => {
  it('returns data and errors keys for poles', async () => {
    const { data, errors } = await parsePolesCSVValidated(POLES_CSV, {
      id: 'id', height: 'height', class: 'class', hasTransformer: 'xfmr', latitude: 'latitude', longitude: 'longitude'
    });
  ...
  it('returns data and errors keys for spans', async () => {
    const { data, errors } = await parseSpansCSVValidated(SPANS_CSV, {
      id: 'id', fromId: 'from_id', toId: 'to_id', length: 'length', proposedAttach: 'attach'
    });
    expect(Array.isArray(data)).toBe(true);
    expect(errors).toBeDefined();
  });
  it('produces at least one validation error for invalid numeric values', async () => {
    const poleRes = await parsePolesCSVValidated(POLES_CSV, { id: 'id', height: 'height', class: 'class', hasTransformer: 'xfmr', latitude: 'latitude', longitude: 'longitude' });
    const spanRes = await parseSpansCSVValidated(SPANS_CSV, { id: 'id', fromId: 'from_id', toId: 'to_id', length: 'length', proposedAttach: 'attach' });
