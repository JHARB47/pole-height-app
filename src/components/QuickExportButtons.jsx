...
export default function QuickExportButtons({
  poles,
  spans,
  existingLines,
  job,
  ...
  const onExport = async () => {
    ...
    try {
      await downloadExportZip({
        ...
        preset,
        includeBOM: true,
        onProgress: (p) => setPercent(Math.round(p)),
      });
    } finally {
      setBusy(false);
      setPercent(100);
  ...
  const onKmz = async () => {
    ...
    try {
      const blob = await buildKMZ({
        poles,
        spans,
        name: job?.name || "export",
      });
      const name =
        sanitizeFilename(
          [job?.jobNumber, job?.name || "export"].filter(Boolean).join("-"),
        ) || "export";
    ...
    <div className="flex items-center gap-2">
      <button
        ...
        title={
          !hasData
            ...
            : "Download CSV, GeoJSON, KML, and KMZ as a ZIP bundle"
        }
      >
        {busy
          ? percent
      ...
      <button
        ...
        title={
          !hasData
            ? "No data to export yet"
            : "Download KMZ for Google Earth and similar tools"
        }
      >
        KMZ Only
      </button>
      <button
        ...
        title={
          !hasData
            ? "No data to export yet"
            : "Customize CSV export with regulatory frameworks"
        }
      >
        Custom CSV
      </button>
