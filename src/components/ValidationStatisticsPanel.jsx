        ...
        <div className="px-4 pb-4">
          <details className="group">
            <summary className="cursor-pointer text-sm font-medium text-red-900 hover:text-red-700 flex items-center gap-2">
              ...
              View {stats.errorDetails.length} Error
              {stats.errorDetails.length !== 1 ? "s" : ""}
            ...
            <div className="mt-2 space-y-2 max-h-48 overflow-y-auto">
              {stats.errorDetails.map((detail, idx) => (
                <div
                  key={idx}
                  className="p-2 bg-red-50 rounded text-xs border border-red-200"
                >
        ...
        <div className="px-4 pb-4">
          <details className="group">
            <summary className="cursor-pointer text-sm font-medium text-yellow-900 hover:text-yellow-700 flex items-center gap-2">
              ...
              View {stats.warningDetails.length} Warning
              {stats.warningDetails.length !== 1 ? "s" : ""}
              <span className="text-gray-400 group-open:rotate-90 transition-transform">
                ▶
              </span>
            </summary>
            <div className="mt-2 space-y-2 max-h-48 overflow-y-auto">
              {stats.warningDetails.map((detail, idx) => (
                <div
                  key={idx}
                  className="p-2 bg-yellow-50 rounded text-xs border border-yellow-200"
                >
