  ...
  return !spans.length ? null : (
    <div className="rounded border p-3 print:hidden">
      ...
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <label
          className="inline-flex items-center gap-1 text-sm text-gray-700"
          title="Show only rows where manual vs auto Δ ≥ threshold"
        >
          <input
            type="checkbox"
        ...
        <label
          className="inline-flex items-center gap-1 text-sm text-gray-700"
          title="Highlight Δ badge when manual vs auto length differs by this amount or more."
        >
        ...
        <div className="mt-3">
          ...
          <ul className="list-disc pl-5 text-xs text-gray-700">
            {(store.cachedMidspans || []).map((m) => {
              ...
              return (
                <li key={m.id}>
                  Span {m.spanId}: {m.spanFt} ft — midspan{" "}
                  {m.midspanFt != null ? formatFeetInches(m.midspanFt) : "—"} vs
                  target{" "}
                  {m.targetFt != null ? formatFeetInches(m.targetFt) : "—"} (
