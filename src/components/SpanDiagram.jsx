...
export default function SpanDiagram({ attachFt, midspanFt, spanFt, groundTargetFt }) {
  ...
  const maxH = Math.max(attachFt || 0, (midspanFt || 0) + 5, groundTargetFt || 0, 10);
  const minH = 0;
  ...
  return (
    <svg width={width} height={height} className="border rounded bg-white">
      <g transform={`translate(${margin.left},${margin.top})`}>
        {/* ground line */}
        <line x1={0} x2={innerW} y1={yScale(0)} y2={yScale(0)} stroke="#999" strokeDasharray="4 4" />
        {/* ground clearance target */}
        <line x1={0} x2={innerW} y1={yScale(groundTargetFt || 0)} y2={yScale(groundTargetFt || 0)} stroke="#16a34a" strokeDasharray="6 4" />
        {/* cable sag curve */}
