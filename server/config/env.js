...
export function loadEnv(overrides = {}) {
  if (!cached) {
    ...
    if (dotenvEnabled) {
      loadEnvFile({ path: process.env.ENV_FILE || '.env', override: false });
    }
    ...
    const base = {
      ...
      jwtSecret: ensure(process.env.JWT_SECRET, 'JWT_SECRET', { required: true, fallback: isTest ? 'test-secret' : undefined }),
      refreshTokenSecret: ensure(process.env.REFRESH_TOKEN_SECRET, 'REFRESH_TOKEN_SECRET', { required: true, fallback: isTest ? 'test-refresh-secret' : undefined }),
