...
const NETLIFY_SITE_BASE =
  process.env.NETLIFY_SITE_BASE || "poleplanpro.netlify.app";
...
const DISABLE_PREVIEW_AUTH = /^(1|true|yes)$/i.test(
  String(process.env.DISABLE_PREVIEW_AUTH || "false"),
);
...
function isAllowedReturnOrigin(origin) {
  ...
  try {
    ...
    if (
      host === NETLIFY_SITE_BASE ||
      host.endsWith(`--${SITE_SUBDOMAIN}.netlify.app`)
    )
      return true;
...
router.post("/register", authLimiter, async (req, res) => {
  try {
    const { email, password, first_name, last_name, organization_code } =
      req.body;
...
router.get("/google", (req, res, next) => {
  ...
  if (DISABLE_PREVIEW_AUTH && isPreviewHost(reqHost)) {
    return res
      .status(403)
      .json({
        error: "Preview authentication disabled",
...
router.get("/github", (req, res, next) => {
  ...
  return passport.authenticate("github", { scope: ["user:email"], state })(
    req,
    res,
    next,
  );
