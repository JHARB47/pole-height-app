...
export async function getUserBySsoSubject(subject) {
  ...
  const { rows } = await query("SELECT * FROM users WHERE sso_subject = $1", [
    subject,
  ]);
  return rows[0] || null;
}
...
export async function upsertUserFromSsoProfile({
  organizationId,
  departmentId,
  email,
  displayName,
  ...
  if (existingByEmail) {
    if (!existingByEmail.sso_subject) {
      await query(`UPDATE users SET sso_subject = $2 WHERE id = $1`, [
        existingByEmail.id,
        ssoSubject,
      ]);
