...
export class MetricsService {
  ...
  // Health check method
  getHealthStatus() {
    const errorRate = this.getCounter('http_errors_total') / Math.max(this.getCounter('http_requests_total'), 1);
    const memoryUsage = this.getGauge('memory_usage_bytes');
    const maxMemory = 1024 * 1024 * 1024; // 1GB threshold
    
    return {
      status: errorRate < 0.1 && memoryUsage < maxMemory ? 'healthy' : 'degraded',
