...
const SentryClient = (() => {
  if (isTestLike || isDevDisabled) {
    ...
    const safeSpan = (
      /** @type {(arg0: { setAttribute: () => void; setStatus: () => void; end: () => void; }) => any} */ callback,
    ) => {
    ...
    return {
      ...
      startSpan: (
        /** @type {any} */ _ctxOrCallback,
        /** @type {any} */ maybeCallback,
      ) => {
        // Support startSpan(callback) or startSpan(ctx, callback)
        const callback =
          typeof _ctxOrCallback === "function" ? _ctxOrCallback : maybeCallback;
  ...
  try {
    ...
    console.warn(
      "[Sentry] profiling integration failed to initialize (non-fatal):",
      err && err.message,
    );
  }
  ...
  try {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "[Sentry] startSpan failed during startup (non-fatal):",
        err && err.message,
      );
    }
    ...
    // eslint-disable-next-line no-console
    console.error(
      "[Sentry] Initialization failed, using no-op client:",
