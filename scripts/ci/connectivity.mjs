...
async function main() {
  ...
  console.log(
    `Mode: ${required ? "required (will fail on warnings)" : "warn-only (never fails on warnings)"}\n`,
  );
  ...
  ghaAnnotate(
    ...
    `ok=${okCount} warn=${warnCount} skipped=${skippedCount}`,
  );
  ...
  if (
    ...
    if (warnMessages.length) {
      ...
      if (warnMessages.length > 25)
        md += `- …and ${warnMessages.length - 25} more\n`;
      md += "\n";
    }
    ...
    if (skippedMessages.length) {
      ...
      if (skippedMessages.length > 25)
        md += `- …and ${skippedMessages.length - 25} more\n`;
      md += "\n";
    }
