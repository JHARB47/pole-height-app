...
const RUN_OPTIONAL_CHECKS =
  (
    process.env.RUN_OPTIONAL_CHECKS ??
    (DEPLOY_ENV === "production" ? "true" : "false")
  )
    .toString()
    .toLowerCase() === "true";
...
const HEALTH_URL =
  process.env.DEPLOY_HEALTH_URL || "http://localhost:3001/health";
...
const CHECKS = [
  {
    ...
    command:
      process.env.DEPLOY_CHECK_NODE_VERSION ||
      `node -e "const v=process.version.slice(1).split('.').map(Number); const r='${REQUIRED_NODE_VERSION}'.slice(1).split('.').map(Number); process.exit(v[0]>r[0]||(v[0]===r[0]&&v[1]>=r[1])?0:1)"`,
  },
  {
    id: "postgres-ready",
    label: "PostgreSQL Ready",
    command:
      process.env.DEPLOY_CHECK_POSTGRES ||
  ...
  },
  {
    ...
    command:
      process.env.DEPLOY_CHECK_SSL ||
      `test -f ${SSL_CERT_PATH} && test -f ${SSL_KEY_PATH}`,
  },
  {
...
const shouldRunCheck = (checkId, isRequired, isOptional) => {
  ...
  if (isOptional && !RUN_OPTIONAL_CHECKS) {
    console.log(
      `• Skipping ${checkId} (optional in ${DEPLOY_ENV}; set RUN_OPTIONAL_CHECKS=true to run)`,
    );
    return false;
  }
...
if (failures.length === 0) {
  ...
  console.log("\nTips:");
  console.log(
    "  • Set DEPLOY_ENV=staging to relax database/SSL/test requirements.",
  );
  console.log(
    "  • Set RUN_OPTIONAL_CHECKS=true to force optional checks even in staging.",
  );
  console.log(
    '  • Provide SKIP_DEPLOY_CHECKS="check-id,other-id" to bypass specific checks.',
  );
