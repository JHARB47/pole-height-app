...
class ProductionChecker {
  ...
  checkEnvironmentVariables() {
    ...
    requiredEnvVars.forEach(({ name, production, optional }) => {
      ...
      if (!value && !optional) {
        ...
        this.log(
          `  ⚠ ${name}: ${value} (should be ${production} in production)`,
          colors.yellow,
        );
      } else {
  ...
  checkBuildArtifacts() {
    ...
    try {
      const files = execSync('find dist -name "*.map" 2>/dev/null', {
        encoding: "utf-8",
      });
      if (files.trim()) {
        this.warnings.push(
          "Source maps found in dist/ (consider removing for production)",
        );
  ...
  checkBundleSize() {
    ...
    try {
      ...
      const sizeKB = parseInt(
        execSync(`du -sk ${distPath}`, { encoding: "utf-8" }).split("\t")[0],
      );
      if (sizeKB > 2048) {
  ...
  printReport() {
    ...
    console.log(
      `${colors.red}Critical Issues: ${this.critical.length}${colors.reset}\n`,
    );
