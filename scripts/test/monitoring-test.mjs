...
try {
  const envOutput = execSync('npx netlify env:list --json', { encoding: 'utf8' });
...
try {
  const healthResponse = execSync('curl -s "https://poleplanpro.com/.netlify/functions/health"', { encoding: 'utf8' });
  ...
  if (healthData.ok && healthData.performance) {
    ...
    console.log(`   Memory usage: ${healthData.performance.memoryUsage.heapUsed}MB heap used`);
  } else {
...
console.log('\n3. Testing Main Site...');
try {
  const siteResponse = execSync('curl -s -I "https://poleplanpro.com" | head -1', { encoding: 'utf8' });
...
try {
  ...
  if (indexHtml.includes('sentry')) {
    ...
    console.log('⚠️  Sentry configuration not found in build (may be runtime only)');
  }
} catch (error) {
...
try {
  ...
  if (perfResponse.trim()) {
    ...
    console.log('⚠️  Performance monitoring code not detected (may load asynchronously)');
  }
} catch (error) {
...
console.log('- Netlify Analytics: https://app.netlify.com/sites/poleplanpro/analytics');
console.log('- UptimeRobot: https://uptimerobot.com');
console.log('- Health Check: https://poleplanpro.com/.netlify/functions/health');