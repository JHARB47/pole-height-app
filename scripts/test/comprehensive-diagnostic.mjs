...
async function checkEnvironment() {
  await test('Node version', async () => {
    ...
    return { status: 'warn', message: `Node ${version} (recommended: 20.x - 22.x)` };
  });
...
async function checkTests() {
  await test('Unit Tests', async () => {
    const result = await runCommand('npm', ['run', 'test:ci'], { silent: true });
    if (result.code === 0) {
  ...
  await test('API Tests', async () => {
    const result = await runCommand('npm', ['run', 'test:api'], { silent: true });
    if (result.code === 0) {
...
async function checkBuild() {
  await test('Production Build', async () => {
    ...
    if (result.code === 0) {
      ...
      if (checkFile('dist/index.html')) {
        return { status: 'pass', message: 'Build successful, dist/index.html created' };
      }
      return { status: 'warn', message: 'Build completed but dist/index.html not found' };
    }
  ...
  await test('Bundle Size Check', async () => {
    const result = await runCommand('npm', ['run', 'check:bundle'], { silent: true });
    if (result.code === 0) {
      return { status: 'pass', message: 'Bundle size within limits' };
    }
    return { status: 'warn', message: 'Bundle size check failed or exceeded limits' };
  });
}
...
async function checkDependencies() {
  await test('Dependencies Installed', async () => {
    if (checkFile('node_modules')) {
      const result = await runCommand('npm', ['list', '--depth=0'], { silent: true });
  ...
  await test('Audit Dependencies', async () => {
    const result = await runCommand('npm', ['audit', '--audit-level=high'], { silent: true });
    if (result.code === 0) {
...
async function checkCalculations() {
  await test('Calculation Functions Available', async () => {
    ...
    const missing = functions.filter(fn => !content.includes(`export function ${fn}`));
    if (missing.length > 0) {
      return { status: 'fail', message: `Missing functions: ${missing.join(', ')}` };
    }
    
    return { status: 'pass', message: 'All core calculation functions present' };
  });
}
...
async function checkDatabase() {
  await test('Database Schema', async () => {
    ...
    return { status: 'warn', message: 'Schema file exists but may be incomplete' };
  });
}
...
async function checkGitStatus() {
  await test('Git Repository', async () => {
    ...
    const result = await runCommand('git', ['status', '--porcelain'], { silent: true });
