...
console.log(
  `${result1.success ? "✅" : "❌"} Utility Standard: ${result1.success ? "Created" : result1.error}`,
);
...
console.log(
  `${result2.success ? "✅" : "❌"} Field Survey Quick: ${result2.success ? "Created" : result2.error}`,
);
...
console.log(
  `${result3.success ? "✅" : "❌"} Compliance Review: ${result3.success ? "Created" : result3.error}`,
);
...
const testCoordinates = [
  ...
  { name: "New York City", lat: 40.7128, lon: -74.006, expected: "valid" },
  ...
  sampleProjects.push({
    ...
    project_data: {
      poles: [
        ...
        {
          id: `P${i}-2`,
          latitude: 37.7749 + i * 0.01 + 0.001,
          longitude: -122.4194 + i * 0.01 + 0.001,
          height: 45,
        },
      ],
    },
...
console.log(
  `  - Organizations: ${new Set(sampleProjects.map((p) => p.organization_id)).size}`,
);
console.log(
  `  - Clients: ${new Set(sampleProjects.map((p) => p.client_id)).size}`,
);
...
console.log(
  '   curl -H "Authorization: Bearer $TOKEN" "http://localhost:3000/api/projects?search=San&page=1&limit=10"\n',
);
console.log("   # Filter by organization");
console.log(
  '   curl -H "Authorization: Bearer $TOKEN" "http://localhost:3000/api/projects?organization_id=org_1"\n',
);
