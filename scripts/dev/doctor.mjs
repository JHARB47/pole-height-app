...
async function main() {
  ...
  const nodeOk =
    compareVersions(node, "22.0.0") >= 0 && compareVersions(node, "25.0.0") < 0;
  log(
    nodeOk,
    `Node version ${node} (expected >=22 <25${reqNode ? `; recommended ${reqNode}` : ""})`,
  );
  ...
  if (!skip) {
    ...
    log(
      true,
      "NETLIFY_NEXT_PLUGIN_SKIP defaulted to true for local tooling (add to .env to persist)",
    );
  } else {
  ...
  log(
    hasDb,
    "Database URL detected in environment (optional for health check)",
  );
  // Ports
  ...
  log(
    p8888Free,
    p8888Free
      ? "Port 8888 is free (netlify dev can bind)"
      : "Port 8888 is in use (netlify dev may already be running)",
  );
  log(
    p5173Free,
    p5173Free
      ? "Port 5173 is free (Vite can bind)"
      : "Port 5173 is in use (Vite may already be running)",
  );
  log(
    hasDb,
    "Database URL detected in environment (optional for health check)",
  ...
  if (!p8888Free && typeof fetch === "function") {
    try {
      ...
      const res = await fetch(
        "http://127.0.0.1:8888/.netlify/functions/health",
        { signal: ac.signal },
      );
      clearTimeout(t);
      if (res.ok) {
        ...
        log(
          true,
          `Functions health reachable on 8888 (ok=${String(data?.ok)})`,
        );
        if (!hasDb) {
          console.log(
            "   Tip: set DATABASE_URL in .env to test db_test function.",
          );
        }
      } else {
      ...
      log(
        false,
        `Could not reach http://127.0.0.1:8888/.netlify/functions/health (${errMsg})`,
      );
    }
  }
  ...
  try {
    ...
    const v = execSync("npx --yes netlify --version", {
  ...
  console.log(
    'If curl shows "exit code 7", the server is not listening on that port. Check the dev logs for the actual port.',
  );
}
