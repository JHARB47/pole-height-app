...
async function main() {
  ...
  if (!distWritable) {
    ...
    console.error('This usually happens if a previous build was run with sudo, causing root-owned files.');
  ...
  try {
    ...
    if (!manifestWritable) {
      console.error('Build blocked: dist/.vite/manifest.json exists but is not writable.');
      console.error('Fix (macOS/Linux):');
      console.error('  sudo chown -R "$(id -un)":"$(id -gn)" dist');
      process.exit(1);
    }
