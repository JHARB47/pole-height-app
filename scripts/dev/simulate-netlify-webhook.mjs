...
const file = argv.find(a => !a.startsWith('--')) || 'scripts/dev/fixtures/netlify.deploy.json';
...
if (shouldPost) {
  ...
  console.log(`\n# POSTing to ${endpoint} ...`);
  const res = await fetch(endpoint, { method: 'POST', headers: postHeaders, body: payload });
