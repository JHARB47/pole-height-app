...
function analyzeCodeComplexity() {
  ...
  try {
    ...
    log.info(`Average lines per file: ${Math.round(totalLines / totalFiles)}\n`);
    
    if (complexFiles.length > 0) {
      log.warn('Files exceeding 300 lines (consider refactoring):');
      complexFiles.sort((a, b) => b.lines - a.lines).forEach(({ file, lines }) => {
...
function runAnalysis() {
  ...
  log.success('Review recommendations above to optimize application performance\n');
}
