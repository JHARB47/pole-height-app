...
for (const file of potentialUnusedFiles) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    try {
      const result = execSync(`grep -r "${file}" . --exclude-dir=node_modules --exclude="*.log" --exclude="${file}" || true`, 
...
if (fs.existsSync(redirectsFile)) {
  ...
  if (content.includes('#')) {
    console.log('⚠️  public/_redirects contains markdown-like content but should be plain text');
  }
}
