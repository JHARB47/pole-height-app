...
export class DatabaseTestHelper {
  ...
  async createTestUser(userData = {}) {
    const defaultUser = {
      ...
      password_hash:
        "$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz7q8kQ6y8NnY4KxW7R8Y4rJ3J5Y7Yq", // 'password'
      first_name: "Test",
      last_name: "User",
      role: "user",
      is_active: true,
...
describe("Database Integration Tests", () => {
  ...
  test("should create project with geospatial data", async () => {
    ...
    const projectData = {
      ...
      user_id: user.id,
      project_data: {
        spans: [{ id: 1, length: 100, height: 30 }],
      },
      location: {
        type: "Point",
  ...
  test("should handle database transactions", async () => {
    ...
    await expect(
      dbTestHelper.db.transaction(async (client) => {
        await client.query(
          "INSERT INTO projects (name, description, user_id) VALUES ($1, $2, $3)",
    ...
    const projectsAfterRollback = await dbTestHelper.db.getProjectsByUser(
      user.id,
    );
